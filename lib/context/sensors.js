...
import { locationContext } from './location.js';
import { systemResourcesContext } from './system-resources.js';
...
export function sense(opts = {}) {
  ...
  // System resources (adaptive complexity)
  const sysRes = systemResourcesContext();
  hints.push(sysRes.summary);
  if (sysRes.complexity.level !== 'full') hints.push(sysRes.complexity.hint);

  // News context (tech trends from HN)
  const news = newsContext();
  if (news) hints.push(news);

  // Location context (geo-aware design)
  let location = null;
  try {
    // locationContext is async but we handle gracefully
    const locPromise = locationContext();
    if (locPromise && typeof locPromise.then === 'function') {
      // sync fallback — cache will be populated next call
    }
  } catch {}
